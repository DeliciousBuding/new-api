...
describe('user settings saves across profile and security', () => {
  it('a partial settings patch is merged onto the latest complete settings', async () => {
    ...
    await updateUserSettings({ webhook_url: 'https://example.com/updated' })
  ...
  it('saving settings for a user with no stored settings supplies the existing defaults', async () => {
    ...
    const put = vi
      .spyOn(api, 'put')
      .mockResolvedValue({ data: { success: true } })
    await updateUserSettings({ upstream_model_update_notify_enabled: true })
    expect(put).toHaveBeenCalledWith('/api/user/setting', {
      notify_type: 'email',
  ...
  it('alternating saves from different pages retain the latest value of each field', async () => {
    let saved = { ...settings }
    ...
    await updateUserSettings({ webhook_secret: 'rotated-secret' })
    await updateUserSettings({ quota_warning_threshold: 3000 })
    expect(saved).toEqual({
      ...settings,
      quota_warning_threshold: 3000,
  ...
  // Fork contract: audit-log IP capture is a single global admin switch, so the
  // notification form must keep its own edits and expose no per-user IP toggle.
  it('saving the notification form keeps its edits and exposes no per-user IP toggle', async () => {
