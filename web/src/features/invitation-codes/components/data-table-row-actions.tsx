...
import { invitationCodeSchema } from '../types'
...
export function DataTableRowActions<TData>({
  ...
  const invitationCode = invitationCodeSchema.parse(row.original)
  const { setOpen, setCurrentRow, triggerRefresh } = useInvitationCodes()
  const isEnabled = invitationCode.status === INVITATION_STATUS.ENABLED
  const isExpired = isInvitationExpired(
    invitationCode.expired_time,
    invitationCode.status
  )
  const isExhausted = isInvitationExhausted(
