...
export async function getInvitationCodes(
  ...
  const res = await api.get(
    `/api/invitation_code/?p=${p}&page_size=${page_size}`
  )
  return res.data
}
...
export async function searchInvitationCodes(
  ...
  const res = await api.get(
    `/api/invitation_code/search?${queryParams.toString()}`
  )
  return res.data
}
...
export async function createInvitationCode(
  ...
  const res = await api.post('/api/invitation_code/', data)
  return res.data
}
...
export async function updateInvitationCode(
  ...
  const res = await api.put('/api/invitation_code/', data)
  return res.data
}
...
export async function updateInvitationCodeStatus(
  ...
  const res = await api.put('/api/invitation_code/?status_only=true', {
    id,
    status,
  })
  return res.data
...
// Delete invalid invitation codes (disabled, expired, exhausted)
