# VERSION is supplied as a build-arg by CI (docker-build.yml passes the release
# tag); local builds fall back to "dev" so no committed VERSION file can drift
# out of sync with the latest release tag.
ARG VERSION=dev
